...
@section('content')
    ...
    @php
        ...
        if (!empty($artwork->location)) {
            ...
            if (!empty($artwork->gallery_number))
                $galleryText .= ', Gallery ' . $artwork->gallery_number;
        }
            ...
            $related = \App\Models\ArtWork::where('art_work_id', '!=', $artwork->art_work_id)
                ->where(function ($q) use ($artwork) {
                    if ($artwork->department_id)
                        $q->orWhere('department_id', $artwork->department_id);
                    if ($artwork->classification_id)
                        $q->orWhere('classification_id', $artwork->classification_id);
                })
            ...
            <div class="met-wrap met-hero__grid">
                    ...
                    <div class="met-quickmeta">
                                ...
                                <div class="met-qm-row">
                                    ...
                                    <span class="met-qm-val">
                                        @if($qLabel === 'Department')
                                            <a href="{{ route('art.search', ['department[]' => $qVal]) }}"
                                                style="color: var(--met-black); text-decoration: underline; text-decoration-color: #bbb; text-underline-offset: 3px; cursor: pointer; transition: text-decoration-color 0.15s;"
                                                onmouseover="this.style.textDecorationColor='var(--met-black)';"
                                                onmouseout="this.style.textDecorationColor='#bbb';">{{ $qVal }}</a>
                                        @else
                                            {{ $qVal }}
                                        @endif
                                    </span>
                        ...
                        @if($galleryText)
                            <div class="met-qm-row">
                                ...
                                <span class="met-qm-val met-qm-val--gallery">On view at
                                    <strong>{{ $galleryText }}</strong></span>
                            </div>
                        @endif
                    </div>
                ...
                <div class="met-hero__gallery" id="met-gallery">
                    ...
                    <div class="met-imgframe" id="met-imgframe" onclick="metLightboxOpen()">
                        @if($primaryImage)
                            <img id="met-mainimg" src="{{ $resolveUrl($primaryImage) }}" alt="{{ $artwork->title }}"
                                class="met-mainimg">
                    ...
                    @if($allImages->count() > 1)
                        <div class="met-thumbs">
                            ...
                            @foreach($allImages as $img)
                                ...
                                <button class="met-thumb {{ $u === $primaryUrl ? 'met-thumb--on' : '' }}"
                                    onclick="metSetImg('{{ $u }}')" type="button" data-url="{{ $u }}">
                                    <img src="{{ $u }}" alt="">
                                </button>
                            @endforeach
                        </div>
                    ...
                    <div class="met-lightbox" id="met-lightbox" style="display:none" onclick="metLightboxClose()">
                        ...
                        <img id="met-lightbox-img" src="{{ $resolveUrl($primaryImage) }}" class="met-lightbox__img"
            ...
            <div class="met-wrap">
                ...
                <div class="met-details__layout">
                    ...
                    <nav class="met-sidenav" id="met-sidenav" aria-label="Artwork sections">
                        <button class="met-sidenav__btn met-sidenav__btn--active" data-tab="overview"
                            onclick="metTab('overview')">Overview</button>
                        @if($showSims)
                            <button class="met-sidenav__btn" data-tab="sims" onclick="metTab('sims')">Signatures, Inscriptions
                                &amp; Markings</button>
                        @endif
                        @if($showProvenance)
                            <button class="met-sidenav__btn" data-tab="provenance"
                                onclick="metTab('provenance')">Provenance</button>
                        @endif
                        @if($showExhibition)
                            <button class="met-sidenav__btn" data-tab="exhibition" onclick="metTab('exhibition')">Exhibition
                                History</button>
                        @endif
                        @if($showReferences)
                            <button class="met-sidenav__btn" data-tab="references"
                                onclick="metTab('references')">References</button>
                        @endif
                    </nav>
                    ...
                    <div class="met-panel">
                                        ...
                                        <div class="met-dl__row">
                                            ...
                                            <dd class="met-dl__dd">
                                                @if($rowLabel === 'Curatorial Department')
                                                    <a href="{{ route('art.search', ['department[]' => $rowValue]) }}"
                                                        style="color: var(--met-black); text-decoration: underline; text-decoration-color: #bbb; text-underline-offset: 3px; cursor: pointer; transition: text-decoration-color 0.15s;"
                                                        onmouseover="this.style.textDecorationColor='var(--met-black)';"
                                                        onmouseout="this.style.textDecorationColor='#bbb';">{{ $rowValue }}</a>
                                                @elseif($rowLabel === 'Classification')
                                                    <a href="{{ route('art.search', ['object_type[]' => $rowValue]) }}"
                                                        style="color: var(--met-black); text-decoration: underline; text-decoration-color: #bbb; text-underline-offset: 3px; cursor: pointer; transition: text-decoration-color 0.15s;"
                                                        onmouseover="this.style.textDecorationColor='var(--met-black)';"
                                                        onmouseout="this.style.textDecorationColor='#bbb';">{{ $rowValue }}</a>
                                                @else
                                            ...
                                            <div class="met-sim-group">
                                                ...
                                                @foreach($simGroup as $sim)
                                                    <div class="met-sim-entry">
                                                        <span
                                                            class="met-sim-badge met-sim-badge--{{ strtolower($simLabel) }}">{{ $simLabel }}</span>
                            ...
                            @if($provenance !== '')
                                <div class="met-provenance-wrap">
                                    <p class="met-provenance-note">
                                        The following is the ownership history of this work from the time of its creation or
                                        earliest recorded existence to its acquisition by the Metropolitan Museum of Art.
                                    </p>
                        ...
                        <div class="met-tab-panel" id="tab-exhibition" style="display:none">
                            @if($exhibitions->isNotEmpty())
                                <p class="met-section-subtitle">{{ $exhibitions->count() }}
                                    exhibition{{ $exhibitions->count() !== 1 ? 's' : '' }} on record</p>
                                        ...
                                        <li class="met-timeline__item">
                                            ...
                                            <div class="met-timeline__body">
                                                ...
                                                @if(!empty($ex->venue_name) || !empty($ex->city_name))
                                                    <span
                                                        class="met-timeline__venue">{{ implode(', ', array_filter([$ex->venue_name ?? '', $ex->city_name ?? ''])) }}</span>
                                                @endif
                        ...
                        <div class="met-tab-panel" id="tab-references" style="display:none">
                            @if($references->isNotEmpty())
                                <p class="met-section-subtitle">{{ $references->count() }}
                                    reference{{ $references->count() !== 1 ? 's' : '' }} on record</p>
